...
async function testarFluxoBonus() {
  try {
    ...
    if (servicesError) {
      log(
        `⚠️  Tabela services não existe ou erro: ${servicesError.message}`,
        "yellow",
      );
      log(
        `\n📋 Execute o SQL: supabase/migrations/create_services_data.sql`,
        "cyan",
      );
    ...
    log(
      `✅ Cliente atualizado: R$ ${newTotal.toFixed(2)}, ${newPoints} pontos`,
      "green",
    );
