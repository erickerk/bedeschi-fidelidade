...
async function fixRLS() {
  ...
  console.log('👥 Clientes (anon):', clientsErr ? `❌ ${clientsErr.message}` : `✅ ${clients?.length} registros`);
  ...
  console.log('📋 Regras (anon):', rulesErr ? `❌ ${rulesErr.message}` : `✅ ${rules?.length} registros`);
  ...
  console.log('🎁 Recompensas (anon):', rewardsErr ? `❌ ${rewardsErr.message}` : `✅ ${rewards?.length} registros`);
  ...
  console.log('📅 Agendamentos (anon):', aptErr ? `❌ ${aptErr.message}` : `✅ ${appointments?.length} registros`);

  console.log('\n--- Dados com service key ---');
  
  // Testar com service key
  const { data: sClients } = await supabase.from('fidelity_clients').select('id, name, phone');
  const { data: sRules } = await supabase.from('fidelity_rules').select('id, name');
  const { data: sRewards } = await supabase.from('fidelity_rewards').select('id, title');
