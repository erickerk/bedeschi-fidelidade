...
async function cleanTables() {
  console.log(
    "🧹 Limpando tabelas de dados fake (preservando usuários criados)...\n",
  );
...
async function seedAppointments(clients) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (hasReview) {
      ...
      if (rand < 0.1)
        rating = Math.random() < 0.5 ? 1 : 2; // 10% ruins
      else if (rand < 0.3)
        rating = 3; // 20% médias
  ...
  for (const apt of appointments) {
    ...
    if (error) {
      ...
      console.log(
        `   ✅ ${apt.date} - ${services.map((s) => s.name).join(", ")} (R$ ${apt.total})`,
      );
...
async function updateClientStats(clients, appointments) {
  ...
  for (const client of clients) {
    ...
    const pointsBalance = clientAppts.reduce(
    ...
    if (error) {
      ...
      console.log(
        `   ✅ ${client.name}: ${totalAppointments} atendimentos, R$ ${totalSpent}, ${pointsBalance} pts`,
      );
    }
  }
}
...
async function seedRewards(clients) {
  ...
  const rewardTypes = [
    ...
    {
      title: "Massagem Grátis",
      description: "Uma massagem relaxante de 30 minutos",
      type: "FREE_SERVICE",
      value: 0,
      service_name: "Massagem Relaxante 30min",
    },
    {
      title: "20% de Desconto",
      description: "Desconto de 20% no próximo serviço",
