...
async function corrigirDados() {
  try {
    ...
    for (const cliente of clientes) {
      ...
      if (numAtendReais === 0 && (cliente.total_appointments > 0 || cliente.total_spent > 0 || cliente.points_balance > 0)) {
        log(`🔧 Zerando dados fictícios de: ${cliente.name} (${cliente.phone})`, 'yellow');
        log(`   Antes: ${cliente.total_appointments} atendimentos, R$ ${cliente.total_spent}, ${cliente.points_balance} pontos`, 'yellow');
        ...
        const gastoReal = atendimentosCompletos?.reduce((sum, a) => sum + parseFloat(a.total), 0) || 0;
        const pontosReais = atendimentosCompletos?.reduce((sum, a) => sum + a.points_earned, 0) || 0;

        if (cliente.total_spent !== gastoReal || cliente.points_balance !== pontosReais || cliente.total_appointments !== numAtendReais) {
          ...
          if (error) {
            ...
            log(`   ✅ Corrigido: ${numAtendReais} atendimentos, R$ ${gastoReal.toFixed(2)}, ${pontosReais} pontos`, 'green');
          }
        } else {
