...
if (!supabaseUrl || !serviceRoleKey) {
  console.error(
    "❌ Erro: Configure NEXT_PUBLIC_SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY",
  );
  process.exit(1);
}
...
async function runMigration() {
  ...
  if (checkError && checkError.code === "42P01") {
    console.log(
      "❌ Tabela roles não existe. Execute o SQL manualmente no dashboard.",
    );
    console.log(
      "\n📋 Copie o conteúdo de: supabase/migrations/000_full_migration.sql",
    );
    console.log(
      "   Cole em: https://supabase.com/dashboard/project/lvqcualqeevdenghexjm/sql/new",
    );
    return false;
  }
...
async function validate() {
  ...
  for (const p of profiles) {
    ...
    console.log(
      `   ${status} ${p.full_name} (${p.email}) - ${role?.name || "N/A"}`,
    );
  }
