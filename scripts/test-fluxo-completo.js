  ...
  try {
    ...
    if (profError) {
      ...
      profissionais.forEach((p, i) => {
        log(
          `   ${i + 1}. ${p.name} (${p.role}) - ${p.specialty || "Sem especialidade"}`,
          "blue",
        );
      });
    ...
    if (!resultados.profissional) {
      log(
        "\n❌ TESTE ABORTADO: Cadastre profissionais antes de continuar",
        "red",
      );
      return;
    }
    ...
    const atendimentoTeste = {
      ...
      has_review: false,
      services: [
        { service_name: "Massagem Relaxante 60min", price: 180.0 },
        { service_name: "Limpeza de Pele", price: 170.0 },
      ],
    };
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        points_balance:
          resultados.cliente.points_balance + novoAtendimento.points_earned,
    ...
    const avaliacaoTeste = {
      ...
      rating: 5,
      comment:
        "Atendimento excelente! Profissional muito atencioso e cuidadoso. Recomendo!",
    };
    ...
    if (avalError) {
      ...
      if (markError) {
        log(
          `⚠️  Erro ao marcar atendimento como avaliado: ${markError.message}`,
          "yellow",
        );
      } else {
    ...
    log(
      `   💰 Gasto total: R$ ${clienteAtualizado.total_spent.toFixed(2)}`,
      "blue",
    );
    log(`   ⭐ Pontos: ${clienteAtualizado.points_balance}`, "blue");
    log(
      `   📅 Total de atendimentos: ${clienteAtualizado.total_appointments}`,
      "blue",
    );

    if (clienteAtualizado.total_spent !== 350.0) {
      log(
        `⚠️  ATENÇÃO: Gasto esperado R$ 350.00, mas está R$ ${clienteAtualizado.total_spent}`,
        "yellow",
      );
      testePassed = false;
    }
    ...
    if (clienteAtualizado.points_balance !== 350) {
      log(
        `⚠️  ATENÇÃO: Pontos esperados 350, mas está ${clienteAtualizado.points_balance}`,
        "yellow",
      );
      testePassed = false;
    }
    ...
    log(
      `   ${atendimentoAtualizado.has_review ? "✅" : "❌"} Avaliado: ${atendimentoAtualizado.has_review}`,
      atendimentoAtualizado.has_review ? "green" : "red",
    );

    if (!atendimentoAtualizado.has_review) {
      log(
        `⚠️  ATENÇÃO: Atendimento deveria estar marcado como avaliado`,
        "yellow",
      );
      testePassed = false;
    }
    ...
    if (avaliacaoVerificada) {
      ...
      log(
        `   💬 Comentário presente: ${avaliacaoVerificada.comment ? "Sim" : "Não"}`,
        "blue",
      );
    } else {
    ...
    if (avaliacoesProfissional && avaliacoesProfissional.length > 0) {
      ...
      const somaNotas = avaliacoesProfissional.reduce(
        (sum, r) => sum + r.rating,
        0,
      );
      ...
      log(
        `   ${mediaNotas >= 4.5 ? "🏆 EXCELENTE!" : mediaNotas >= 4.0 ? "👍 BOM" : "⚠️  PRECISA MELHORAR"}`,
        mediaNotas >= 4.5 ? "green" : mediaNotas >= 4.0 ? "yellow" : "red",
      );
    } else {
      log(
        `ℹ️  Nenhuma avaliação encontrada para ${resultados.profissional.name}`,
        "blue",
      );
    }
    ...
    if (testePassed) {
      ...
      log(
        `   📱 Cliente: ${clienteTeste.phone} / PIN: ${clienteTeste.pin}`,
        "blue",
      );
