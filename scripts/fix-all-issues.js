...
async function seedAppointments() {
  ...
  for (const apt of appointments) {
    ...
    if (error) {
      ...
      const { error: svcErr } = await supabase.from('fidelity_appointment_services').insert([
...
async function createServicesViaRPC() {
  ...
  const { error: checkErr } = await supabase.from('services').select('id').limit(1);
...
`);
  ...
  if (!checkErr) {
    ...
    if (!existing || existing.length === 0) {
      const services = [
        { name: 'Massagem Relaxante 60min', category_name: 'Massagens', price: 180.00 },
        { name: 'Limpeza de Pele', category_name: 'Facial', price: 150.00 },
        { name: 'Depilação Perna Completa', category_name: 'Depilação', price: 120.00 },
        { name: 'Design de Sobrancelhas', category_name: 'Sobrancelhas', price: 65.00 }
      ];
...
async function verifyAll() {
  ...
  const checks = [
    ...
    { name: 'fidelity_appointment_services', table: 'fidelity_appointment_services' },
