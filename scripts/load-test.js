...
async function simulateUserSession(userId) {
  ...
  for (let i = 0; i < REQUESTS_PER_USER; i++) {
    ...
    try {
      ...
      console.log(
        `[User ${userId}] Request ${i + 1}/${REQUESTS_PER_USER} - ${responseTime}ms`,
      );
    } catch (error) {
...
async function runLoadTest() {
  ...
  console.log(
    `   Tempo total:           ${totalTime}ms (${(totalTime / 1000).toFixed(2)}s)`,
  );
  ...
  if (results.errors.length > 0) {
    ...
    results.errors.slice(0, 5).forEach((err, i) => {
      console.log(
        `   ${i + 1}. User ${err.userId}, Request ${err.request}: ${err.error}`,
      );
    });
