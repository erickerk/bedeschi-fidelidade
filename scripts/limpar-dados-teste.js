...
async function limparDadosTeste() {
  try {
    ...
    if (clientesTeste && clientesTeste.length > 0) {
      log(
        `\n⚠️  Clientes de TESTE encontrados (${clientesTeste.length}):`,
        "yellow",
      );
    ...
    const { error: errAppointments, count: deletedAppointments } =
      await supabase
        .from("appointments")
        .delete({ count: "exact" })
        .in("client_id", idsClientesTeste);
    ...
    log(
      `   Atendimentos: ${countAtendimentos} → ${countAtendimentosDepois}`,
      "blue",
    );
    ...
    log(
      `   ✅ ${deletedAppointments || 0} atendimento(s) removido(s)`,
      "green",
    );
