...
async function testarFluxoAvaliacao() {
  try {
    ...
    if (existingClient) {
      ...
      await supabase.from('reviews').delete().eq('customer_id', existingClient.id);
      await supabase.from('rewards').delete().eq('client_id', existingClient.id);
      await supabase.from('appointments').delete().eq('client_id', existingClient.id);
    ...
    log(`   ⚠️  has_review: ${atendimento.has_review} (deve ser FALSE)`, 'yellow');
    ...
    log(`\n📊 ESTADO ATUAL:`, 'cyan');
    log(`   🎁 Bônus disponíveis: ${bonusDisponiveis?.length || 0}`, bonusDisponiveis?.length > 0 ? 'green' : 'blue');
    log(`   ⚠️  Atendimentos sem avaliação: ${atendimentosPendentes?.length || 0}`, atendimentosPendentes?.length > 0 ? 'yellow' : 'green');
    ...
    if (bonusDisponiveis?.length > 0 && atendimentosPendentes?.length > 0) {
      ...
      log(`   Com o fix aplicado, o app agora FORÇA avaliação primeiro!`, 'green');
    }
    ...
    log(`\n📊 Total de avaliações com profissional: ${todasAvaliacoes?.length || 0}`, 'cyan');
    ...
    if (todasAvaliacoes && todasAvaliacoes.length > 0) {
      ...
      for (const [staffId, data] of Object.entries(porProfissional)) {
        ...
        log(`   • ${staff?.name || 'Desconhecido'}: ${media} ⭐ (${data.count} avaliações)`, 'blue');
      }

      log(`\n✅ Dados suficientes para gráficos de performance!`, 'green');
    } else {
      log(`⚠️  Sem avaliações com staff_id - gráficos não funcionarão`, 'yellow');
    }
