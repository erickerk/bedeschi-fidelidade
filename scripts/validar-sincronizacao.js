  ...
  try {
    ...
    if (clientesError) {
      ...
      for (const cliente of clientes) {
        ...
        const gastoReal = atendimentos?.reduce((sum, a) => sum + parseFloat(a.total), 0) || 0;
        const pontosReais = atendimentos?.reduce((sum, a) => sum + a.points_earned, 0) || 0;

        // Verificar consistência
        if (cliente.total_appointments !== numAtendimentos) {
          erros.push(`Total atendimentos: esperado ${numAtendimentos}, registrado ${cliente.total_appointments}`);
        }

        if (Math.abs(parseFloat(cliente.total_spent) - gastoReal) > 0.01) {
          erros.push(`Gasto total: esperado R$ ${gastoReal.toFixed(2)}, registrado R$ ${parseFloat(cliente.total_spent).toFixed(2)}`);
        }

        if (cliente.points_balance !== pontosReais) {
          erros.push(`Pontos: esperado ${pontosReais}, registrado ${cliente.points_balance}`);
        }
        ...
        if (erros.length > 0) {
          ...
          log(`✅ ${cliente.name}: Dados consistentes (${numAtendimentos} atendimentos, R$ ${gastoReal.toFixed(2)}, ${pontosReais} pontos)`, 'green');
        }
      }
    }
    ...
    if (atendError) {
      ...
      for (const atendimento of atendimentos) {
        ...
        if (atendimento.has_review !== temReview) {
          log(`⚠️  Inconsistência: Atendimento ${atendimento.id.slice(0, 8)}...`, 'yellow');
        ...
        if (temReview) {
          ...
          if (atendimento.review_rating !== review.rating) {
            log(`⚠️  Nota diferente: atendimento tem ${atendimento.review_rating}, review tem ${review.rating}`, 'yellow');
      ...
      log(`   ⏳ Pendente avaliação: ${semAvaliacao}`, semAvaliacao > 0 ? 'yellow' : 'green');
    }
    ...
    if (reviewsOrfas && reviewsOrfas.length > 0) {
      log(`⚠️  ${reviewsOrfas.length} avaliação(ões) sem atendimento`, 'yellow');
    ...
    if (todosOk && problemas.length === 0) {
      ...
      log('\n💡 Recomendação:', 'cyan');
      log('   Execute o script de correção ou revise os dados manualmente', 'blue');
    }
