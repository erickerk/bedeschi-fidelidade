...
async function fixRLS() {
  ...
  const { data: clients } = await supabase.from('fidelity_clients').select('id, name').limit(1);
  ...
  const { data: allClients } = await supabase.from('fidelity_clients').select('id');
  const { data: allRules } = await supabase.from('fidelity_rules').select('id');
  const { data: allRewards } = await supabase.from('fidelity_rewards').select('id');
  const { data: allAppointments } = await supabase.from('fidelity_appointments').select('id');
