...
async function testarFiltroClienteBonus() {
  try {
    ...
    clientes.forEach((c, i) => {
      log(`   ${i + 1}. ${c.name} - R$ ${c.total_spent} (${c.points_balance} pts)`, 'blue');
    });
    ...
    const bonusDisponiveis = todosBonus?.filter(b => b.status === 'available') || [];
    const bonusResgatados = todosBonus?.filter(b => b.status === 'redeemed') || [];

    log(`\n🎁 Bônus Disponíveis: ${bonusDisponiveis.length}`, bonusDisponiveis.length > 0 ? 'green' : 'yellow');
    ...
    log(`\n✅ Bônus Utilizados: ${bonusResgatados.length}`, bonusResgatados.length > 0 ? 'green' : 'yellow');
    ...
    log(`   Progresso Atual: R$ ${progressoAtual.toFixed(2)} (${percentualProgresso.toFixed(1)}%)`, 'blue');
    log(`   Falta: R$ ${faltaParaProximo.toFixed(2)}`, faltaParaProximo < 100 ? 'yellow' : 'blue');

    if (percentualProgresso >= 80) {
      log(`\n⚠️  PRÓXIMO DO BÔNUS! Cliente está a ${percentualProgresso.toFixed(0)}% do próximo prêmio!`, 'yellow');
    }
    ...
    if (clientesSemDados && clientesSemDados.length > 0) {
      log(`⚠️  ${clientesSemDados.length} cliente(s) com dados incompletos:`, 'yellow');
      clientesSemDados.forEach(c => {
        log(`   • ${c.name}: gasto=${c.total_spent}, pontos=${c.points_balance}`, 'yellow');
      });
    } else {
    ...
    testCases.forEach(test => {
      ...
      if (correto) {
        log(`   ✅ R$ ${test.gasto} → ${percentual.toFixed(1)}% (falta R$ ${falta.toFixed(2)})`, 'green');
      } else {
        log(`   ❌ R$ ${test.gasto} → ${percentual.toFixed(1)}% (esperado ~${test.esperado}%)`, 'red');
        erros++;
      }
    });
    ...
    if (bonusDisponiveis.length > 0) {
      log(`   • Tem ${bonusDisponiveis.length} bônus disponível(is) para usar`, 'green');
    }
    ...
    log(`   Texto: Faltam R$ ${faltaParaProximo.toFixed(2)} para 10% OFF`, 'blue');
    ...
    if (clientesSemDados && clientesSemDados.length > 0) problemas.push(`${clientesSemDados.length} clientes com dados incompletos`);
