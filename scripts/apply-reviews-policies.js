...
async function applyPolicies() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (e1) {
        ...
        console.log('   ✅ Acesso à tabela OK (políticas podem já estar ativas)');
      }
    } else {
  ...
  // Verificar reviews
  const { data, error } = await supabase.from('fidelity_reviews').select('id, rating');
  if (error) {
    console.log(`\n   ❌ Erro ao buscar reviews: ${error.message}`);
    console.log('\n   ⚠️  AÇÃO NECESSÁRIA:');
    console.log('   1. Acesse: https://supabase.com/dashboard/project/lvqcualqeevdenghexjm/editor');
