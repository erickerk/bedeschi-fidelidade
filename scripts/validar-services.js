...
async function validarServices() {
  try {
    ...
    if (count === expectedCount) {
      log(
        `✅ Total correto: ${count} serviços (esperado: ${expectedCount})`,
        "green",
      );
    } else {
      log(
        `⚠️  Total: ${count} serviços (esperado: ${expectedCount})`,
        "yellow",
      );
    }
    ...
    byCategory.forEach((s) => {
      categoryCounts[s.category_name] =
        (categoryCounts[s.category_name] || 0) + 1;
    });
    ...
    if (priceError) {
      ...
      priceRanges.forEach((s) => {
        if (!ranges[s.category_name]) {
          ...
          ranges[s.category_name].min = Math.min(
            ranges[s.category_name].min,
            s.price,
          );
          ranges[s.category_name].max = Math.max(
            ranges[s.category_name].max,
            s.price,
          );
        }
      });
      ...
      Object.entries(ranges).forEach(([cat, range]) => {
        log(
          `💰 ${cat}: R$ ${range.min.toFixed(2)} - R$ ${range.max.toFixed(2)}`,
          "blue",
        );
      });
    }
    ...
    expensive.forEach((s, i) => {
      log(
        `   ${i + 1}. ${s.name} - R$ ${s.price} (${s.category_name})`,
        "blue",
      );
    });
    ...
    if (count === expectedCount && allCorrect && duplicates === 0) {
      ...
      if (count !== expectedCount) {
        log(
          `   ⚠️  Total incorreto: ${count} (esperado: ${expectedCount})`,
          "yellow",
        );
      }
      if (!allCorrect) {
