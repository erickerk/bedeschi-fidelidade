...
async function gerarDadosValidados() {
  try {
    ...
    log(
      `✅ Cliente atualizado: R$ ${total1}, ${Math.floor(total1)} pts, 1 atendimento`,
      "green",
    );
    ...
    log(
      `✅ Cliente atualizado: R$ ${totalGeral2}, ${Math.floor(totalGeral2)} pts, 2 atendimentos`,
      "green",
    );
    ...
    log(
      `✅ Cliente atualizado: R$ ${total4}, ${Math.floor(total4)} pts, 1 atendimento`,
      "green",
    );
    ...
    for (const cliente of todosClientes) {
      ...
      log(
        `   🏥 Atendimentos cadastrados: ${atendimentos?.length || 0}`,
        "blue",
      );
      ...
      if (atendimentos) {
        atendimentos.forEach((apt, idx) => {
          log(
            `      ${idx + 1}. R$ ${apt.total} - ${apt.status} - Avaliado: ${apt.has_review ? "SIM ✅" : "NÃO ⚠️"}`,
      ...
      log(
        `   🎁 Bônus: ${bonus?.length || 0}`,
        bonus && bonus.length > 0 ? "green" : "blue",
      );
      ...
      log(
        ...
        reviews && reviews.length > 0 ? "green" : "yellow",
      );

      if (reviews && reviews.length > 0) {
        reviews.forEach((r) => {
          log(
            `      • ${r.rating} ⭐ - "${r.comment?.substring(0, 30) || "Sem comentário"}..."`,
            "green",
          );
        });
      }
      ...
      if (cliente.total_appointments !== (atendimentos?.length || 0)) {
        log(
          `   ❌ ERRO: total_appointments (${cliente.total_appointments}) ≠ atendimentos reais (${atendimentos?.length})`,
          "red",
        );
      } else {
      ...
      if (atendimentosAvaliados !== avaliacoesReais) {
        log(
          `   ❌ ERRO: Atendimentos avaliados (${atendimentosAvaliados}) ≠ avaliações (${avaliacoesReais})`,
          "red",
        );
      } else {
    ...
    log(
      `\n   2. Cliente 3 (pendente): http://localhost:3005/c/bedeschi`,
      "blue",
    );
