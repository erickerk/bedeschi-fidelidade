  ...
  try {
    ...
    const { data: avaliacao } = await supabase
      ...
      .insert({
        ...
        rating: 5,
        comment:
          "Atendimento excelente! Muito satisfeito com os procedimentos.",
      })
      .select()
      .single();
    ...
    log(
      `   Gasto: R$ ${clienteFinal.total_spent}`,
      clienteFinal.total_spent > 0 ? "green" : "red",
    );
    log(
      `   Pontos: ${clienteFinal.points_balance}`,
      clienteFinal.points_balance > 0 ? "green" : "red",
    );
    log(
      `   Atendimentos: ${clienteFinal.total_appointments}`,
      clienteFinal.total_appointments > 0 ? "green" : "red",
    );
    ...
    log(
      `   Avaliado: ${atendimentoFinal.has_review ? "Sim" : "Não"}`,
      atendimentoFinal.has_review ? "green" : "red",
    );
    log(
      `   Nota: ${atendimentoFinal.review_rating || "N/A"}`,
      atendimentoFinal.review_rating ? "green" : "yellow",
    );
