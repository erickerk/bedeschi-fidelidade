...
async function applyMigration() {
  ...
  const migrationPath = path.join(
    __dirname,
    "../supabase/migrations/005_create_services_table.sql",
  );
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        "⚠️ RPC não disponível, tentando criar tabela diretamente...",
      );
      ...
      if (tables !== null) {
        ...
        console.log(
          "📋 Tabela precisa ser criada manualmente no Supabase Dashboard.",
        );
  ...
  if (insertError) {
    ...
    if (insertError.message.includes("does not exist")) {
      ...
      console.log(
        "   Acesse: https://supabase.com/dashboard/project/lvqcualqeevdenghexjm/sql",
      );
      console.log(
        "   E execute o SQL do arquivo: supabase/migrations/005_create_services_table.sql",
      );
    }
  } else {
