  ...
  try {
    ...
    if (pendingAppts && pendingAppts.length > 0) {
      ...
      pendingAppts.forEach((a) => {
        log(
          `   • ${a.client_name} - ${a.date} (ID: ${a.id.slice(0, 8)}...)`,
          "cyan",
        );
      });
    } else {
    ...
    if (reviewedAppts && reviewedAppts.length > 0) {
      ...
      reviewedAppts.forEach((a) => {
        log(
          `   • ${a.client_name} - Nota: ${a.review_rating || "N/A"} ⭐`,
          "cyan",
        );
      });
    } else {
    ...
    if (novoTeste) {
      ...
      if (appts && appts.length > 0) {
        ...
        appts.forEach((a, i) => {
          ...
          log(
            `     ${i + 1}. ${a.date} - R$ ${a.total} - Avaliado: ${a.has_review ? "Sim" : "NÃO"}`,
            a.has_review ? "green" : "yellow",
          );
      ...
      if (bonuses && bonuses.length > 0) {
        ...
        bonuses.forEach((b) => {
          log(
            `     • ${b.title} - ${b.status}`,
            b.status === "available" ? "green" : "yellow",
          );
        });
      }
    } else {
