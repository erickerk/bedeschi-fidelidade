...
async function validacaoFinalCompleta() {
  ...
  try {
    ...
    for (const cliente of clientes) {
      ...
      if (atendimentos && atendimentos.length > 0) {
        atendimentos.forEach((apt, idx) => {
          log(
            `\n   ${idx + 1}. Atendimento ${apt.id.substring(0, 8)}...`,
            "blue",
          );
          ...
          log(
            `      Avaliado: ${apt.has_review ? "SIM ✅" : "NÃO ⚠️"}`,
      ...
      const atendimentosAvaliados =
        atendimentos?.filter((a) => a.has_review).length || 0;

      log(`\n⭐ AVALIAÇÕES:`, "cyan");
      log(
        `   Atendimentos marcados como avaliados: ${atendimentosAvaliados}`,
        "blue",
      );
      ...
      if (avaliacoes && avaliacoes.length > 0) {
        avaliacoes.forEach((rev, idx) => {
          ...
          log(
            `      Profissional: ${rev.staff_id ? "ID " + rev.staff_id.substring(0, 8) + "... ✅" : "NÃO INFORMADO ❌"}`,
            rev.staff_id ? "green" : "red",
          );
          log(
            `      Comentário: "${rev.comment?.substring(0, 40) || "Sem comentário"}..."`,
            "blue",
          );
        });
      ...
      const bonusDisponiveis =
        bonus?.filter((b) => b.status === "available").length || 0;
      const bonusResgatados =
        bonus?.filter((b) => b.status === "redeemed").length || 0;

      log(`\n🎁 BÔNUS:`, "cyan");
      log(`   Total: ${bonus?.length || 0}`, "blue");
      log(
        `   Disponíveis: ${bonusDisponiveis}`,
        bonusDisponiveis > 0 ? "green" : "blue",
      );
      log(
        `   Resgatados: ${bonusResgatados}`,
        bonusResgatados > 0 ? "green" : "blue",
      );
      ...
      log(
        `   Progresso para bônus: ${percentual.toFixed(1)}% (falta R$ ${faltaParaProximo.toFixed(2)})`,
        "blue",
      );
      ...
      if (atendimentosAvaliados < totalAtendimentos && bonusDisponiveis > 0) {
        ...
        log(`\n   ⚠️  ${aviso}`, "yellow");
        log(
          `      → Cliente deve avaliar primeiro antes de ver bônus!`,
          "yellow",
        );
      }

      // VALIDAÇÃO 6: Cálculo de pontos
      const somaAtendimentos =
        atendimentos?.reduce((sum, a) => sum + (a.total || 0), 0) || 0;
    ...
    if (cliente03) {
      ...
      const temAtendimentoPendente = apt03?.some(
        (a) => a.status === "completed" && !a.has_review,
      );

      log(`   Atendimentos: ${apt03?.length || 0}`, "blue");
      log(`   Avaliações: ${rev03?.length || 0}`, "blue");
      log(
        `   Tem pendente: ${temAtendimentoPendente ? "SIM ⚠️" : "NÃO"}`,
        temAtendimentoPendente ? "yellow" : "green",
      );

      if (temAtendimentoPendente) {
        log(`   ✅ Cliente 03 está PENDENTE de avaliação (correto!)`, "green");
        log(
          `   → No login, deve forçar avaliação antes de ver dashboard`,
          "yellow",
        );
      } else {
    ...
    log(`\n📝 CHECKLIST:`, "cyan");
    log(
      `   ${erros.length === 0 ? "✅" : "❌"} Atendimentos sincronizados`,
      erros.length === 0 ? "green" : "red",
    );
    log(
      `   ${erros.length === 0 ? "✅" : "❌"} Avaliações completas com staff_id`,
      erros.length === 0 ? "green" : "red",
    );
    log(
      `   ${erros.length === 0 ? "✅" : "❌"} Totais financeiros corretos`,
      erros.length === 0 ? "green" : "red",
    );
    log(
      `   ${avisos.length === 0 ? "✅" : "⚠️ "} Regras de negócio`,
      avisos.length === 0 ? "green" : "yellow",
    );
